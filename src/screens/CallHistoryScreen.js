...
const CallHistoryScreen = ({ onBack, isSubScreen = false }) => {
        ...
        <View style={styles.cardHeader}>
          ...
          <View style={[styles.statusBadge, { backgroundColor: isSuccess ? '#ECFDF5' : '#FEF2F2' }]}>
            <Text style={[styles.statusText, { color: isSuccess ? '#059669' : '#DC2626' }]}>
              {isSuccess ? t('success') : item.callStatus}
            </Text>
  ...
  return (
    <View style={styles.container}>
      {!isSubScreen && <ScreenHeader title={t('call_history')} subtitle={t('call_history_desc')} onBack={onBack} />}
      {loading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.empty}>
              ...
              <Text style={styles.emptyTitle}>{t('no_calls')}</Text>
              <Text style={styles.emptyDesc}>{t('call_history_desc')}</Text>
            </View>
          }
        />
      )}
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>{t('call_details')}</Text>
              ...
              <ScrollView showsVerticalScrollIndicator={false}>
                <View style={styles.detailUserCard}>
                  ...
                  <View style={[styles.statusTag, { marginTop: 8, backgroundColor: colors.successBg, paddingHorizontal: 12 }]}>
                    <Text style={[styles.statusText, { color: colors.success, fontSize: 11 }]}>{t('successful')} {t('consultations')}</Text>
                  </View>
                </View>

                <View style={styles.detailSection}>
                  <DetailRow label={t('call_type')} value={selectedItem.call_type == 11 ? t('video_call') : t('voice_call')} icon={selectedItem.call_type == 11 ? "videocam-outline" : "call-outline"} />
